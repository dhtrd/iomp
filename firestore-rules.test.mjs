...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  ...
  await setDoc(doc(d, 'sessions/s_gate'), { name:'بوابة مفتوحة', status:'open', started:true, assignedCounters:['u_ct'], location:'فرع أ', createdBy:'u_owner' });
  await setDoc(doc(d, 'sessions/s_gate/counts/g1'), { code:'A', qty:1 });
});
await T('بعد الإغلاق فقط: المطّلع يُمنع من عدّات جلسة مفتوحة', assertFails(getDocs(collection(db('u_vw'), 'sessions/s_gate/counts'))));
await T('بعد الإغلاق فقط: المطّلع يُمنع من لقطة الجلسة المفتوحة', assertFails(getDocs(collection(db('u_vw'), 'sessions/s_gate/snapshot'))));
...
await T('بعد الإغلاق فقط: العدّاد المكلَّف يظل يقرأ جلسته المفتوحة', assertSucceeds(getDocs(collection(db('u_ct'), 'sessions/s_gate/counts'))));
await T('بعد الإغلاق فقط: المالك (حوكمة) يظل يقرأ المفتوحة', assertSucceeds(getDocs(collection(db('u_owner'), 'sessions/s_gate/counts'))));
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  const d = ctx.firestore();
  await setDoc(doc(d, 'config/permissions'), { viewersAfterClose: false }, { merge: true });
});
await T('إطفاء الإعداد يعيد السلوك: المطّلع يقرأ المفتوحة', assertSucceeds(getDocs(collection(db('u_vw'), 'sessions/s_gate/counts'))));
